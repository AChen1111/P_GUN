...
- Addressables hot-update groups are limited to `Room`, `Buff`, `Item`, `Enemy`, `Weapon`, and `Shared`; these groups keep `Prevent Updates` enabled, and `Shared` only stores duplicated cross-group dependencies such as common bullet prefabs, gun sprites, audio, fonts, and shared render assets.
