...
All assets that should be predownloaded during Root startup must carry the shared `hot_update` label. Category labels such as `room`, `buff`, `item`, `enemy`, `weapon`, `shared`, and `hotfix` are still used for querying and organization, but startup update checks should depend on `hot_update` so adding a new hot-update group does not require runtime code changes. `AddressablesLocalGroupSetup` automatically adds `hot_update` to entries it manages; manually added Addressables entries must follow the same rule.

`Root` is the first Build Settings scene, followed by `StartScene` and `GameScene`. It owns the explicit scene singletons `DataBaseManager`, `LuaManager`, `AddressableLoader`, and `RootHotUpdateController`. Addressables `DisableCatalogUpdateOnStartup` is enabled so `RootHotUpdateController` owns the update UI timing. The boot flow initializes Addressables, checks and updates catalogs, downloads the shared `hot_update` label, executes `hotfix/main` through `LuaManager`, then loads `StartScene`. Databases, room graphs, item prefabs, and weapon prefabs are loaded later by the concrete systems that need them. Network/catalog/download failures may log and continue with built-in or cached content; missing databases or required runtime content are configuration errors and should fail loudly.
